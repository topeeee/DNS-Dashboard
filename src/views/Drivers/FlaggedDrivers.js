...
const FlaggedDrivers = ({getDrivers, drivers, driver, isLoading, error,  approveDriver}) => {
  ...
  useEffect(()=> {
    if(drivers && !formData) {
      setPosts(drivers.sort((a, b) => parseFloat(b.id) - parseFloat(a.id)).filter((user) => user.status === "3"))
    }
  },[drivers, formData]);
